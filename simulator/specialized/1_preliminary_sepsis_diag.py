...
def objective(trial,train_x,train_y,val_x,val_y,test_x,test_y,model_path,kforder):
    params = {
        ...
        # 'scale_pos_weight': trial.suggest_float('scale_pos_weight',1.2, 2.5),#这个更关注学习正样本，灵敏度比特异度高
        'scale_pos_weight': trial.suggest_float('scale_pos_weight', 0.99,1.1),#为了让训练的模型更符合真实医生的规律
        'max_delta_step': trial.suggest_float('max_delta_step', 5, 10.0),  # subsample参数
    }
