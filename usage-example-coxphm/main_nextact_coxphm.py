...
def get_nextact_model(fn_pkl,flag,model_sort,diag_modelname):
    ...
    test_em, test_label = coxphm_test_nextact(data)
    test_uuid = data.uuid
    ...
    test_truedoc_next = test_label
    return  model,test_uuid,test_patid,test_truedoc_next,test_virdoc_next
...
def get_true_virdoc_acc(log,df_sample,combine_param):
    ...
    virdoc_nextact_list = []
    test_uuid, test_patid,test_truedoc_nextact, test_virdoc_nextact =combine_param
    for uuid in test_uuid:
        
        truedoc = test_truedoc_nextact[test_uuid.index(uuid)]
        virdoc = test_virdoc_nextact[test_uuid.index(uuid)]

        truedoc_nextact_list.append(truedoc/8)
        virdoc_nextact_list.append(virdoc/8)
