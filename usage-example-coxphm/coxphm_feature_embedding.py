...
def first_embedding(df,dict_patient_embeddings,model_sort,topath_coxphm,feature_numdim,clickseq):
        ...
        if model_sort == 'sepsis':#保留患者信息
            ...
            if clickseq:
                ...
                if doctor_seq_label is not None:  # 不能合并到上面的条件
                    ...
                    with open(topath_coxphm, 'ab') as file:
...
def get_nextact_by_uuid(uuid_list_coxphm,nextact_list_coxphm,uuid):
    if uuid in uuid_list_coxphm:
        index = uuid_list_coxphm.index(uuid)
        return nextact_list_coxphm[index]
    return 0

def final_embedding(df, dict_patient_embeddings, model_sort, topath_coxphm,feature_numdim):
    uuid_list = []
    nextact_list = []
    uuid_list_coxphm = []
    nextact_list_coxphm = []
...
def model_final_input_emb(model_sort,flag,feature_numdim,pca_dim,clickseq):
    ...
    if flag == 'final' and model_sort != 'sepsis':
        df = pd.read_csv(to_root + f'data_0321_7000_{model_sort}_nextact_coxphm.csv', encoding='gbk')
    else:
    ...
    if clickseq:
        topath_coxphm = to_root + f'{flag}_data_7000_dim_{feature_numdim}_clickseq_coxphm.pkl'
    else:
        topath_coxphm = to_root + f'{flag}_data_7000_dim_{feature_numdim}_coxphm.pkl'
    if os.path.exists(topath_coxphm):
        os.remove(topath_coxphm)

    if flag == 'first':
        first_embedding(df, dictionary, model_sort, topath_coxphm,feature_numdim,clickseq)
    else:
        final_embedding(df, dictionary, model_sort, topath_coxphm,feature_numdim)
