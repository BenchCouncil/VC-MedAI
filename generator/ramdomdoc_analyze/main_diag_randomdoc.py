...
def get_diag_model(fn_pkl,flag,model_sort,diag_modelname):
    ...
    save_ramdoc_diag_predict(data_copy.uuid, randomdoc_diag)
    return model,data_copy.uuid,data_copy.patient_id,randomdoc_diag #真实医生的诊断结果，随机医生的预测结果


def save_ramdoc_diag_predict(uuids,randomdoc_nexts):
    data_to_save = (uuids,randomdoc_nexts)
    if ramdoc_diag_file is None:
        return None
    if not os.path.exists(ramdoc_diag_file):
        with open(ramdoc_diag_file, 'wb') as file:
...
def virdoc_first_diag():
    ...
    _,test_uuid,test_patient_id,test_virdoc_diag = get_diag_model(fn_pkl,'first','diag',first_diag_modelname)
    combine_param = (test_uuid,test_patient_id,test_virdoc_diag)
...
def virdoc_final_diag():
    ...
    _,test_uuid,test_patient_id,test_virdoc_diag = get_diag_model(fn_pkl,'final','diag',final_diag_modelname)
    combine_param = (test_uuid,test_patient_id,test_virdoc_diag)
    return combine_param
...
def get_true_virdoc_acc(log,df_sample,combine_param):
    randomdoc_acc_list = []
    test_uuid, test_patid, randomdoc_diag =combine_param
    for uuid in test_uuid:
        ...
        randomdoc = randomdoc_diag[test_uuid.index(uuid)]
        if randomdoc == pat_label:
            randomdoc_acc_list.append(1)
        else:
            randomdoc_acc_list.append(0)
