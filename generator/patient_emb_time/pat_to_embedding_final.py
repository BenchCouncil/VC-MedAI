...
def write_pkl(fn):
    ...
    final_patient_embedding = root+f'patient_embedding_final.pkl'

    # 读取患者特征嵌入
    final_unique_id_list, final_patient_embedding_list = read_patient_emb(final_patient_embedding)

    df = df[df['UNIQUE_ID'].isin(list(set(final_unique_id_list)))]
    ...
    def process_row(args):
        ...
        final_cat_embedding = row_to_embedding(row)
        if  final_cat_embedding.shape != (1,2474):
            ...
            with open(root + f'patient_embedding_final_test.pkl', 'ab') as file:  # 追加写入
...
if __name__ == '__main__':
    ...
    print(f"患者最终特征嵌入 程序运行时间: {elapsed_minutes:.2f} 分钟，也就是{end_time - start_time}秒")
