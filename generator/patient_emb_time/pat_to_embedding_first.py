...
def row_to_embedding(row):
    ...
    df_first_time_series =  row_to_timeseries(row)
    first_note_dict = row_to_notes(row)
    first_jpg_dict = row_to_jpg(row)
...
def write_pkl(fn):
    ...
    first_patient_embedding = root+f'patient_embedding_first.pkl'

    # 读取患者特征嵌入
    first_unique_id_list, first_patient_embedding_list = read_patient_emb(first_patient_embedding)
    print(first_unique_id_list)
    df = df[df['UNIQUE_ID'].isin(list(set(first_unique_id_list)))]
    print(f'嵌入的患者有{len(df)}')
    ...
    def process_row(args):
        ...
        first_cat_embedding = row_to_embedding(row)
        if first_cat_embedding.shape != (1, 2474):
            ...
            with open(root + f'patient_embedding_first_test.pkl', 'ab') as file:  # 追加写入
...
if __name__ == '__main__':
    ...
    print(f"患者初步特征嵌入 程序运行时间: {elapsed_minutes:.2f} 分钟，也就是{end_time - start_time}秒")
