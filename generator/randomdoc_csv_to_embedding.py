    ...
    def process_row(args):
        ...
        if first_cat_embedding.shape != (1, 2474) or final_cat_embedding.shape != (1,2474):
            ...
            if unique_id not in first_unique_id_list:
                ...
                with open(root+f'patient_embedding_first_randomdoc.pkl', 'ab') as file: # 追加写入
                    pickle.dump(first_to_save, file)

            if unique_id not in final_unique_id_list:
                final_to_save = (unique_id, final_cat_embedding)
                with open(root+f'patient_embedding_final_randomdoc.pkl', 'ab') as file: # 追加写入
